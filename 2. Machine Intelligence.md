[Link to paper](https://scontent-frt3-2.xx.fbcdn.net/v/t39.8562-6/79078865_2602377539850898_574457313268596736_n.pdf?_nc_cat=109&ccb=1-7&_nc_sid=ae5e01&_nc_ohc=jOyoLQh8NDYAX9M_K1i&_nc_ht=scontent-frt3-2.xx&oh=00_AT-Vx_nuIaJLxCGKyAieC_4RQwT4HYzJknOYEk2c1lDdmA&oe=62D1CDDC)

# A Roadmap towards Machine Intelligence (Paper) #
...
To master basic communication skills, the machine will have to understand the concept of positive and negative reward, and develop complex strategies to deal with novel linguistic inputs. This may require discovery of algorithms, and the ability to remember facts skills and even learning strategies.

In order to translate, the machine needs to store pairs of words. The number of pairs is unknown and a flexible growing mechanism may be required.

The more specialized and narrow the functionality of the machine is, the less learning is required. This follows our intuition that, for very specialized forms of behavior, it is possible to program the solution manually. However, as we move to machines with open-ended goals, we need to rely more on general learning from a limited number of examples. The curent state of the art in machine learning is somewhere in the middle of this hierarchy. Tasks such as automatic speech recognition, classification of objects in images or machine translation are already too hard to be solved purely through manual programming, and the best systems rely on some form of statistical learning, where parameters of hand-coded models are estimated from large datasets of examples. However, the capabilities of state-of-the-art machine learning systems are severely limited, and only allow a small degree of adaptability of the machine's functionality.

### Long-term memory and compositional learning skills ###

The long-term memory should be able to store facts and algorithms corresponding to learned skills, and be accessed on-demand. In fact, even the ability to learn should be seen as a set of skills that are stored in memory.

The machine should have the capacity to extend itself.

Thanks to compositional learning skills, the machine will be able to combine several existing abilities to create a new one, often on the fly. In this way, a well-functioning intelligent machine will not need a myriad of training examples whenever it faces a slightly new request, but it could succeed given a single example of the new functionality.

### Computational properties of intelligent machines ###

The computational model should be unrestricted, that is, able to represent any pattern in the data.

The intelligent machine needs to be based on a Turing-complete computational model. That is, it has to be able to represent any algorithm in fixed length, just like the Turing machine.

A system that is weaker than Turing-complete cannot represent certain patterns in data efficiently, which in turn means it cannot truly learn them in a general sense.

In a growing computational model [opposed to non-growing with immensely large capacity] the new cells can be connected to those that spawned them, so that the model is naturally able to develop a meaningful topological structure based on functional connectivity.